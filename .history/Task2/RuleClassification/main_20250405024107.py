...
if __name__ == "__main__":
    ...
    model_path = os.path.join("saved_models", config["name"] + ".pt")
    if os.path.exists(model_path):
        ...
        stem_rules = checkpoint["stem_rules"]
        tags = checkpoint["tags"]
        model = model.to(device)
        logger.info("Loaded model successfully. Skipping training...")
        skip_training = True
    else:
        logger.info("No saved model found. Proceeding to training.")
        skip_training = False
    ...
    if not skip_training:
        ...
        start = time.time()  # Start time for consistency
    ...
    if not test:
        ...
        logger.info(f"Precision: {scores.get('task_2_precision', 0):.2f}%")
        logger.info(f"Recall: {scores.get('task_2_recall', 0):.2f}%")
        logger.info(f"F1 Score: {scores.get('task_2_f1score', 0):.2f}%")
        logger.info(f"Task Score: {scores.get('task_2_tscore', 0):.2f}%")
